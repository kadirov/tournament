...
apt-get install -y php7.2-curl php7.2-cli php7.2-intl php7.2-mysqlnd php7.2-gd php7.2-fpm php7.2-mbstring php7.2-xml unzip nginx mariadb-server-10.0 php.xdebug

info "Configure MySQL"
sed -i "s/.*bind-address.*/bind-address = 0.0.0.0/" /etc/mysql/mariadb.conf.d/50-server.cnf
mysql -uroot <<< "CREATE USER 'dbuser'@'%' IDENTIFIED BY 'dbpasswd'"
mysql -uroot <<< "GRANT ALL PRIVILEGES ON *.* TO 'dbuser'@'%'"
...
mysql -uroot <<< "CREATE DATABASE tournament"
mysql -uroot <<< "CREATE DATABASE tournament_test"
echo "Done!"
